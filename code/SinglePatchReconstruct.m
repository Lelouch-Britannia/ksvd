function SinglePatchReconstruct(D, X_test, patch_size, T)
    ...
    %
    % Output:
    %   No return value. (Individual)Visualizations are displayed, and scores are printed to the console.
    ...
    subplot(2,2,1); imshow(patch_original, []); title('Original Patch');
    subplot(2,2,2); imshow(patch_reconstructed, []); title('Reconstructed Patch');
    
    % Stem plots
    subplot(2,2,3); stem(patch_original(:), 'r', 'Marker', 'o'); 
    title('Original Patch Values');
    xlabel('Pixel Index');
    ylabel('Pixel Intensity');
    grid on;
    
    subplot(2,2,4); stem(patch_reconstructed(:), 'b', 'Marker', 'x'); 
    title('Reconstructed Patch Values');
    xlabel('Pixel Index');
    ylabel('Pixel Intensity');
    grid on;
