function results_matrix = gridSearchCombined(low_res_img, patch_sizes, K_values, sparsities, iternum)
    ...
    % INPUTS:
    %   low_res_img   - The low-resolution image used for reconstruction.
    ...
    % USAGE:
    %   results = gridSearchCombined(low_res_img, patch_sizes, K_values, sparsities, iternum, metric);
    %
    ...
    for p = 1:length(patch_sizes)
        ...
        X = preProcessing(low_res_img, current_patch_size);
        ...
        for k = 1:length(K_values)
            for s = 1:length(sparsities)
                ...
                for fold = 1:5
                    ...
                    img = reconstruct_from_patches_2d(X, current_patch_size, size(low_res_img));
                    reconstructed_img = reconstruct_from_patches_2d(Y_pred_all, current_patch_size, size(low_res_img));

                    avg_scores.RMSE = avg_scores.RMSE + evalMetric(Y_true_val, Y_pred_val, 'RMSE');
                    avg_scores.PSNR = avg_scores.PSNR + evalMetric(Y_true_val, Y_pred_val, 'PSNR');
                    avg_scores.SSIM = avg_scores.SSIM + evalMetric(img, reconstructed_img, 'SSIM');
                end
